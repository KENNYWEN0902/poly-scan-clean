...
> 2026-04 CLOB V2 迁移说明：本仓库已切到 `py-clob-client-v2` + `pUSD` 路线。Polymarket 最新公告确认正式切换时间为 2026-04-28 约 11:00 UTC，旧 `py-clob-client` 集成在切换后将失效。

> 2026-04-24 运行修复说明：当前正式项目目录为 `/Users/kennywen/Documents/Playground/polymarketbot`。本次已合并 Chainlink RTDS 连接、Dashboard pUSD 余额、历史页空数据、API 超时缓存和前端 WebSocket 状态修复。
...
- **Chainlink RTDS**：`internal/btc/chainlink_monitor.go` 已更新为 Polymarket RTDS 当前协议，使用 `action/subscriptions/topic/type/filters` 订阅、5 秒 `PING` 保活，并在断开后持续重连。
- **RTDS 严格模式**：`POLY_REQUIRE_CHAINLINK_RTDS=1` 时只接受 `chainlink_rtds` 源；默认 `0` 时允许降级行情但会输出 warning。
- **Dashboard 余额**：`/api/account` 优先通过 Python executor 查询 CLOB collateral/pUSD 余额，带短超时和 20 秒缓存，避免页面被余额接口拖死。
- **Dashboard 稳定性**：前端请求统一 12 秒超时，历史页接受空数组，所有页面不再因为单个接口慢而一直卡在“正在加载”。
- **WebSocket 状态**：Dashboard 首页改为订阅全局 WebSocket，不再重复创建连接导致左下角误报“离线”。
- **运行日志/告警**：API 可从通知事件转换为 Dashboard alerts，便于后续在网页展示运行事件。
...
└─────────────┘     └──────┬───────┘     └──────▲──────┘
                    ...
                    ┌──────▼───────┐     ┌──────┴──────┐
                    │  Python 签名  │     │  REST API   │
                    │ (py-clob-v2) │     │  Port 9876  │
                    └──────────────┘     └─────────────┘
```

- **Go（监听与计算层）**：WebSocket 长连接、实时价格监控、交易信号计算、风险管理
- **Python（执行与签名层）**：EIP-712 签名、Polymarket L2 鉴权（HMAC-SHA256）、`py-clob-client-v2` SDK
...
- **Go 1.18+**
- **Python 3.10+** + `py-clob-client-v2`
- **Node.js 18+**（仪表盘）
- 带有 Polygon 链 `pUSD` 抵押资产的 EVM 钱包
...
```bash
# Python 依赖
pip3 install py-clob-client-v2 requests web3 eth-account eth-abi
...
# 可选：如果你要在 2026-04-28 切换前提前联调 V2，临时指向预发布地址
...
> ⚠️ `pUSD` 已替代 `USDC.e` 成为交易抵押资产。API-only 钱包需要先把 `USDC.e` 包装成 `pUSD`，否则 V2 下单会因为 collateral 不足而失败。
...
| POLY_CLOB_HOST | ❌ | CLOB API 地址；切换前联调可设为 `https://clob-v2.polymarket.com` |
| POLY_BUILDER_CODE | ❌ | CLOB V2 Builder code（bytes32） |
| POLY_RPC_URL | ❌ | 自定义 Polygon RPC，供余额查询和赎回使用 |
| POLY_REQUIRE_CHAINLINK_RTDS | ❌ | 设为 `1` 时要求预测必须使用 RTDS Chainlink 价格源 |
| POLY_DASHBOARD_FETCH_GENERIC_MARKETS | ❌ | 设为 `1` 时 Dashboard API 会额外拉取 Gamma 通用市场列表 |
| API_PORT | ❌ | 仪表盘 API 端口（默认 9876）|

## 项目管理文档

- `REQUIREMENTS.md` / `REQUIREMENTS.html`：当前运行目标、功能需求与验收标准
- `DEVELOPMENT.md` / `DEVELOPMENT.html`：开发、同步、构建、部署与验证流程
- `CHANGELOG.md` / `CHANGELOG.html`：按日期记录重要修复和迁移变更
...
3. **CLOB V2 切换**：2026-04-28 约 11:00 UTC 会有约 1 小时停机，所有挂单会被清空；切换前请提前迁移到 V2 SDK
4. **Gnosis Safe**：需设置 `POLY_SIGNATURE_TYPE=2` 和 `POLY_FUNDER_ADDRESS`
5. **安全提醒**：私钥仅存储在 `.env` 文件中，严禁提交到版本控制
